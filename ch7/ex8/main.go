package main

import (
	"fmt"
	"os"
	"sort"
	"text/tabwriter"
	"time"
)

type Track struct {
	Title  string
	Artist string
	Album  string
	Year   int
	Length time.Duration
}

var tracks = []*Track{
	{"Go", "Delilah", "From the Roots Up", 2012, length("3m38s")},
	{"Go", "Moby", "Moby", 1992, length("3m37s")},
	{"Go Ahead", "Alicia Keys", "As I Am", 2007, length("4m36s")},
	{"Ready 2 Go", "Martin Solveig", "Smash", 2011, length("4m24s")},
}

func length(s string) time.Duration {
	d, err := time.ParseDuration(s)
	if err != nil {
		panic(s)
	}
	return d
}

type TrackLessFunc func(x, y *Track) bool

func TitleLess(x, y *Track) bool  { return x.Title < y.Title }
func ArtistLess(x, y *Track) bool { return x.Artist < y.Artist }
func AlbumLess(x, y *Track) bool  { return x.Album < y.Album }
func YearLess(x, y *Track) bool   { return x.Year < y.Year }
func LengthLess(x, y *Track) bool { return x.Length < y.Length }

func Reverse(l TrackLessFunc) TrackLessFunc {
	return func(x, y *Track) bool {
		return l(y, x)
	}
}

func printTracks(tracks []*Track) {
	const format = "%v\t%v\t%v\t%v\t%v\t\n"
	tw := new(tabwriter.Writer).Init(os.Stdout, 0, 8, 2, ' ', 0)
	fmt.Fprintf(tw, format, "Title", "Artist", "Album", "Year", "Length")
	fmt.Fprintf(tw, format, "-----", "------", "-----", "----", "------")
	for _, t := range tracks {
		fmt.Fprintf(tw, format, t.Title, t.Artist, t.Album, t.Year, t.Length)
	}
	tw.Flush() // calculate column widths and print table
}

func main() {
	sort.Sort(multiTierSort{tracks, []TrackLessFunc{TitleLess, Reverse(YearLess), LengthLess}})
	printTracks(tracks)
}

type multiTierSort struct {
	t        []*Track
	lessFunc []TrackLessFunc
}

func (x multiTierSort) Len() int {
	return len(x.t)
}
func (x multiTierSort) Swap(i, j int) {
	x.t[i], x.t[j] = x.t[j], x.t[i]
}
func (x multiTierSort) Less(i, j int) bool {
	for _, less := range x.lessFunc {
		a, b := x.t[i], x.t[j]
		if !less(a, b) && !less(b, a) {
			continue
		}
		return less(a, b)
	}
	return false
}
